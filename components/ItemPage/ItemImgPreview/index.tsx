...
const ItemImgPreview = ({ images }) => {
  ...
  const disabledDown =
    Math.abs(topPosition) / 88 === images.length - 4 || images.length < 5

  return (
    <div className={`${styles.goodsImgPrevWrap} fl fcb`}>
      <div className={`${styles.leftListWrap} fcb`}>
        <a
          className={`${styles.prevBtn} ${disabledUp ? styles.disabled : ''}`}
          onClick={() => {
            if (disabledUp) {
              return
            }
        ...
        <a
          className={`${styles.prevBtn} ${disabledDown ? styles.disabled : ''}`}
          onClick={() => {
            if (disabledDown) {
              return
            }
      ...
      <div className={`${styles.imgBox}`} id={'box'}>
        ...
        <div className={`hide`} id={'local'}>
          <p className="title">The coordinates of mouse in picture</p>
          <p>
            {' '}
            x : <span></span>
          </p>
